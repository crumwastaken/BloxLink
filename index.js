...
function loadState() {
    try {
        ...
        if (saved.date === getDateKey()) {
            ...
            // Out-of-stock generators are paused, not permanently disabled.
...
async function processGenerator(type, fromStockCheck = false) {
    ...
    try {
        ...
        if (result === "RATE_LIMIT") {
            ...
            console.log(
                `[BLOXGEN] ${displayType(type)} rate limited. Waiting ${formatDuration(wait)}.`
            );
        ...
        if (result === "NO_STOCK") {
            const firstTime = markOutOfStock(type);

            console.log(
                `[STOCK] ${displayType(type)} out of stock. No requests for 5 minutes.`
            );
...
async function generatorLoop(type) {
    ...
    try {
        while (STATE.running && !STATE.stopping) {
            ...
            if (hasReachedLimit(type)) {
                console.log(
                    `[GENERATOR] ${displayType(type)} daily limit reached. Stopping this generator.`
                );
            ...
            if (isOutOfStock(type)) {
                const wait = getStockCheckRemaining(type);
                ...
                if (result === "RATE_LIMIT" || result === "RETRY") {
            ...
            if (wait > 0) {
                console.log(
                    `[COOLDOWN] Waiting ${formatDuration(wait)} for ${displayType(type)}.`
                );
                await sleep(wait);
                continue;
            }
