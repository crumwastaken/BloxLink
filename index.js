...
function markOutOfStock(type) {
    const firstTime = !STATE.outOfStock[type];
    ...
    try {
        while (STATE.running && !STATE.stopping) {
            ...
            if (isOutOfStock(type)) {
                ...
                if (wait > 0) {
                    console.log(
                        `[STOCK] ${displayType(type)} unavailable. Next stock check in ${formatDuration(wait)}.`
                    );
                    await sleep(wait);
                    continue;
                }
                ...
                console.log(
                    `[STOCK] ${displayType(type)} checking for stock...`
                );
