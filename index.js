...
async function processGenerator(type) {
    ...
    if (!config.enabled || isOutOfStock(type) || isDisabled(type) || !STATE.running || STATE.stopping) {
        return;
    }
...
async function generatorLoop(type) {
    while (STATE.running && !STATE.stopping) {
        ...
        if (isOutOfStock(type) || isDisabled(type) || hasReachedLimit(type)) {
            await sleep(1000);
            continue;
        }
        ...
        const cooldown = Math.max(
            ...
            Math.max(0, (STATE.apiRetryUntil[type] || 0) - Date.now())
        );

        if (cooldown > 0) {
            console.log(
                `[COOLDOWN] Waiting ${formatDuration(cooldown)} for ${displayType(type)}.`
            );
